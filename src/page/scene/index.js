...
class Scene {
    ...
    curScore = 0;
    die = false;
    $scene = this.$page.find(".scene-container");
    ...
    moveBg() {
        let speed = 6 + this.level * 2;
        this.bgLeft += speed;
        this.$scene.find(".stone,.gift-container").each((i, dom) => {
            let $dom = $(dom);
            let left = parseInt($dom.css("left"));
            $dom.css("left", left - speed);
        })
    ...
    showStone() {
        ...
        this.$scene.append($stone);

        //clear when die
        function clear() {
            let rect = $stone[0].getBoundingClientRect();
            if (rect.left < -300 || rect.top < -300) {
                $stone.remove();
            } else {
    ...
    showGift() {
        ...
        this.$scene.append($giftContainer);
